...
';
	...
	} else if ($operacion == 'facturaListaDatDiario')
	{
		...
		$sql_listado_facturaxfecha = mysqli_query  ($link, "SELECT * FROM tbl_basediaria WHERE fecha='$fechaHoy' ",$link);	
...
if ($variable == "producto") {
	...
	if ($operacion == 'insertarProveedor') {
		...
		$sql_listadoProducto = mysqli_query  ($link, "SELECT * FROM tbl_producto p,tbl_inventario i WHERE p.id_producto=i.id_producto  order by p.id_producto DESC LIMIT 100 "  );
